-- Migration: Create Agent Network Views
-- Created: 2025-11-08
...
SELECT
  ...
  a.autonomy_level,
  a.metadata,
  COUNT(DISTINCT wa.workflow_id) AS workflow_count,
  COALESCE(SUM(wa.active_instances), 0)::INTEGER AS active_instances
FROM agents a
LEFT JOIN agent_categories ac ON a.category_code = ac.code
LEFT JOIN workflow_agents wa ON a.id = wa.agent_id
WHERE a.active = true
GROUP BY a.id, a.code, a.name, a.category_code, ac.name, ac.icon, ac.color, a.autonomy_level, a.metadata;
...
INSERT INTO agents (code, name, category_code, description, autonomy_level, metadata) VALUES
  ('demand_forecaster', 'Demand Forecaster', 'predictive', 'Predicts passenger demand for routes', 4, '{"capabilities": ["time_series_analysis", "seasonal_patterns"]}'),
  ('dynamic_pricer', 'Dynamic Pricing Agent', 'decisioning', 'Optimizes ticket prices in real-time', 5, '{"capabilities": ["price_optimization", "competitor_analysis"]}'),
  ('chatbot', 'Customer Support Bot', 'conversational', 'Handles customer inquiries 24/7', 3, '{"capabilities": ["natural_language", "multi_language"]}'),
  ('schedule_optimizer', 'Schedule Optimizer', 'decisioning', 'Optimizes flight schedules and crew assignments', 4, '{"capabilities": ["constraint_solving", "resource_allocation"]}'),
  ('baggage_tracker', 'Baggage Tracking Agent', 'monitoring', 'Monitors baggage location and alerts on delays', 2, '{"capabilities": ["iot_integration", "real_time_tracking"]}'),
  ('maintenance_predictor', 'Predictive Maintenance', 'predictive', 'Predicts aircraft maintenance needs', 4, '{"capabilities": ["anomaly_detection", "failure_prediction"]}'),
  ('checkin_automation', 'Check-in Automation', 'automation', 'Automates passenger check-in process', 3, '{"capabilities": ["document_verification", "seat_assignment"]}'),
  ('fraud_detector', 'Fraud Detection', 'monitoring', 'Detects fraudulent transactions and bookings', 4, '{"capabilities": ["pattern_recognition", "risk_scoring"]}'),
  ('crew_scheduler', 'Crew Scheduler', 'automation', 'Automates crew scheduling and assignments', 3, '{"capabilities": ["constraint_solving", "compliance_checking"]}'),
  ('sentiment_analyzer', 'Customer Sentiment Analyzer', 'data_processing', 'Analyzes customer feedback and sentiment', 2, '{"capabilities": ["nlp", "sentiment_analysis"]}')
ON CONFLICT (code) DO NOTHING;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON workflow_agents TO authenticated;
