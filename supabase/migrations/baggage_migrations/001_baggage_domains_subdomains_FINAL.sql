/*
  Migration 001: Baggage Domains & Subdomains (Schema Compatible)

  Purpose: Add 5 core baggage operations domains with 18 subdomains

  This version works with the actual schema (no icon column, only icon_url)
...
DECLARE
  v_domain_id BIGINT;
BEGIN
  -- Insert domain (only using columns that exist)
  INSERT INTO domains (name, description, color, created_at, updated_at)
  VALUES (
  ...
  RAISE NOTICE 'Domain 1: Baggage Operations & Tracking created with 4 subdomains';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Domain 2: Baggage Exception Management created with 4 subdomains';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Domain 3: Interline Baggage Coordination created with 3 subdomains';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Domain 4: Baggage Compensation & Claims created with 3 subdomains';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Domain 5: Baggage Analytics & Optimization created with 4 subdomains';
END $$;
