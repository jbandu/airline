/*
  Migration 005: Baggage Workflow Definitions (Final - Schema Corrected)

  Purpose: Define 30 operational workflows across 5 baggage domains

  SCHEMA FIXES:
  - Uses `implementation_wave` instead of `wave`
  - Uses `complexity` as INTEGER (not TEXT): 2=Low, 3=Medium, 4-5=High
  - Uses `status` as lowercase: 'planned' not 'PLANNED'
  - Removes non-existent columns: business_value, technical_feasibility, effort_estimate, impact_score, priority_score
  - Casts BIGINT subdomain IDs to UUID if needed
